...
package com.hendrix.starling.text.bidiTextField
{
  import flash.geom.Rectangle;
  import flash.utils.Dictionary;
  
  import com.hendrix.starling.text.bidiTextField.bidi.Bidi;
  ...
  public class BidiBitmapFont
  {
    ...
    private var _bidiLevel:uint = 0;
    
    private var _hasTrancted:Boolean = false;
    private var _tranctationPosition:Rectangle = null
    
    /** Creates a bitmap font by parsing an XML file and uses the specified texture. 
     *  If you don't pass any data, the "mini" font will be created. */
    public function BidiBitmapFont(texture:Texture=null, fontXml:XML=null)
    {
      ...
      */  
      ...
      _tranctationPosition  = new Rectangle();
    ...
    public function fillQuadBatch(quadBatch:QuadBatch, width:Number, height:Number, text:String,
                                  ...
                                  hAlign:String="center", vAlign:String="center",      
                                  autoScale:Boolean=true, 
                                  kerning:Boolean=true, isTrancted:Boolean = true):void
    {
      var charLocations:Vector.<CharLocation>; 
      
      _tranctationPosition.setEmpty();
      _hasTrancted = false;
      
      smoothing = TextureSmoothing.BILINEAR;
      //if(name == "Arial")
      charLocations = arrangeCharsBidi(width, height, text, fontSize, hAlign, vAlign, autoScale, kerning, isTrancted);
      //else
      ...
      if (numChars > 8192) {
        ...
        charLocations.length = 8192;
        numChars = charLocations.length;
    ...
    private function arrangeCharsBidi(width:Number, height:Number, text:String, fontSize:Number=-1,
                                      hAlign:String="center", vAlign:String="center",
                                      autoScale:Boolean=true, kerning:Boolean=true, isTrancted:Boolean = true):Vector.<CharLocation>
    {
      ...
      //var bd:bi
      _bidi.bidiMe(text, bidiLevel);
      
      var textOverFlowedHeight:Boolean = false;
      ...
      while (!finished)
      {
        ...
        if (mLineHeight <= containerHeight)
        {
          ...
          for (var i:int=0; i<numChars; ++i)
          {
            ...
            {
              // fallback to CHAR_SPACE if no other available
              if(char == null) {
                charID = (charID == CHAR_SPACE || charID == CHAR_TAB) ? charID : CHAR_SPACE;
                char = getChar(charID);
              }
            ...
            else if (lineFull)
            {
              ...
              {
                textOverFlowedHeight = true;
                break;
                // this can be optimization for bounded text, passing only the chars that are relevent
              }
            }
          } // for each char
        } // if (mLineHeight <= containerHeight)
      ...
      for (var lineID:int=0; lineID<numLines; lineID++)
      {
        ...
        sIndex = charsProcessed;
        
        var cc:int = text.charCodeAt(sIndex);
        if((cc == CHAR_SPACE) || (cc == CHAR_NEWLINE) || (cc == CHAR_CARRIAGE_RETURN) || (cc == CHAR_TAB)){
          sIndex += 1;
        }
        
        eIndex = sIndex + numChars;
        
        textLine = _bidi.reorderLineSegment(sIndex, eIndex);
        
        charsProcessed = eIndex;
        
        currentX = 0;
        
        for(var ix:uint = 0; ix < numChars; ix++)
        {
          charID = textLine.charCodeAt(ix);
          char  = getChar(charID) || getChar(CHAR_SPACE);
          charLocation = line[ix];
          
          if(lineID == numLines - 1) {
            if(textOverFlowedHeight && isTrancted) 
            {
              if(bidiLevel%2 == 1) {
                if(ix < 4) {
                  char = getChar(CHAR_DOT);
                  _hasTrancted = true;
                  if(ix == 0) {
                    _tranctationPosition.x = currentX + char.xOffset;
                    _tranctationPosition.y = currentY + char.yOffset ;//+ char.yOffset;
                    _tranctationPosition.width = mSize;
                    _tranctationPosition.height = scale*(mLineHeight);
                    trace();
                  }
                ...
                if(ix > numChars - 4) {
                  ...
                  if(ix == numChars - 1) {
                    ...
                    _tranctationPosition.x = charLocation.x;
                    _tranctationPosition.y = charLocation.y;
                    _tranctationPosition.width = mSize;
                    _tranctationPosition.height = mLineHeight;
                  }
          ...
          if(char == null) // avoid new line and carriage return
            continue;
      ...
      for (lineID = 0; lineID<numLines; ++lineID)
      {
        ...
        if(lineID == numLines - 1) {
          _tranctationPosition.x = scale * (_tranctationPosition.x + xOffset);
          _tranctationPosition.y = scale * (_tranctationPosition.y + yOffset);
        }
    ...
    public function get tranctationPosition():Rectangle
    {
      return _tranctationPosition;
    }
    
    public function get hasTrancted():Boolean
    {
      return _hasTrancted;
    }
